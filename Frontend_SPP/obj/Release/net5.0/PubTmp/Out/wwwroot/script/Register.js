        ...
        $.ajax({
            ...
            success: function (Result) {
                if (Result.Error == false) {
                    ...
                    if (errorMsg == 'Invalid Credential - 3' || errorMsg == 'Invalid Captcha - 3') {
                        CustomNotif("error|Oops|" + Result.Message + "");
                        ResetFormLogin();
                        FillCapcthaLogin();
                    }
...
function LoadChart3() {
    ...
    $.ajax({
        ...
        success: function (data) {
            var result = data.Message[0];
            debugger;
            var div_KebijakanProsedur = document.getElementById('div_KebijakanProsedur');
            var KebijakanProsedur = document.createElement('span');
            $(KebijakanProsedur).attr('data-from', 0);
            $(KebijakanProsedur).attr('data-to', result.KDP);
            $(KebijakanProsedur).attr('data-speed', 2000);
            $(KebijakanProsedur).attr('data-refresh-interval', 50);
            div_KebijakanProsedur.innerHTML = '';
            div_KebijakanProsedur.appendChild(KebijakanProsedur);
