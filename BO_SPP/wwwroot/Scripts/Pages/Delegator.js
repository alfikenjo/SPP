$(document).ready(function () {
    sessionStorage.setItem("DelegatorID", "");
                ...
                render: function (data, type, full, meta) {
                    return '<td>' +
                        '<div class="flex align-items-center list-user-action text-nowrap">' +
                            ...
                            '<button type="button" class="btn btn-outline-success btn-sm mr-1" onclick="OpenMember(\'' + full.ID + '\');"><i class="fa fa-users mr-1"></i>&nbsp;Member ('+ full.CountMember +')</button>' +
                        '</div>' +
                        '</td>';
...
function RefreshTable() {
    $.ajax({
        url: VP + 'Delegator/Get_Delegator',
        type: 'POST',
...
$("#btnAdd").click(function () {
    $("#Action").val("add");
    $("#FormTitle").text("Tambah Grup Delegator");
    resetFormEdit();
    $("#divInput").show();
...
function OpenMember(ID) {
    sessionStorage.setItem("DelegatorID", ID);
...
function Delete(ID) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: VP + 'Delegator/SaveDelegator',
                data: DataForm,
                type: 'POST',
                contentType: false,
                processData: false,
    ...
    $.ajax({
        ...
        success: function (Result) {
            if (Result.Error == false) {
                ...
                $("#FormTitle").text("Edit Grup Delegator");
                $("#divInput").show();
                document.getElementById("Name").focus();
            } else {
                CustomNotif("error|Oops|" + Result.Message + "");
...
$('#FormInput').validate({
    ...
    submitHandler: function () {
        ...
        DataForm.append('isActive', $('#Status').val());
