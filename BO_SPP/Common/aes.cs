...
namespace BO_SPP.Common
{
    public class aes
    {
        ...
        public static void EncryptDatabase()
        {
            ...
            //if (DateTime.Now.ToString("yyyyMMdd") == "20221107")
            //{
            //    DataTable tblM_Referensi = mssql.GetDataTable("SELECT * FROM tblT_Dumas WHERE NULLIF(Jenis_Pelanggaran, '') IS NOT NULL AND CreatedOn < '2022-10-29'");
            //    foreach (DataRow dr in tblM_Referensi.Rows)
            //    {
            //        string ID = dr["ID"].ToString();
            //        string Jenis_Pelanggaran = aes.Dec(dr["Jenis_Pelanggaran"].ToString());
            //        mssql.ExecuteNonQuery("UPDATE tblT_Dumas SET Jenis_Pelanggaran = '" + Jenis_Pelanggaran + "' WHERE ID = '" + ID + "';");
            //    }
            //}
